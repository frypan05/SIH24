  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100">
      <header className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto py-2 px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button className="text-blue-600 hover:text-blue-800 text-sm" onClick={handleShare}>Share</button>
      ...
      <main className="flex-grow p-4" onClick={handleCanvasClick}>
        ...
        {/* File Upload Section */}
        <div className="mt-4 bg-gray-200 rounded-lg shadow-md p-4"> {/* Changed background color */}
          ...
          {uploadedFiles.map((file, index) => (
            <div key={index} className="mt-4 p-4 bg-white border border-gray-300 rounded-md shadow-sm text-black"> {/* Added text-black */}
              {file.type === 'pdf' && (
                <Document file={file.content} className="pdf-container">
                  <Page pageNumber={1} />
                </Document>
              )}
              {file.type === 'excel' && (
